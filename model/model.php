...
class Model {
    ...
    public function buscarUsuario($usuario, $pass, $latitud, $longitud) {
        ...
        $mongo_usuario=array('usuarios.usuario' => "$usuario", 'usuarios.pass' => "$pass");
        ...
        foreach ( $cursor as $id => $value ) {
            if ($id['validated'] == 1) {
                $usuario=new Usuario($id['id_usuario'],$id["usuario"],$id["pass"]);
                echo "<p>".$usuario->mostrar()."</p>";
                $_SESSION['id_usuario']=$id["id_usuario"];
                
                $titulo=date("Y-m-d H:i:s");
                //Model::insertarPosicion($id['id_usuario'], $titulo, $latitud, $longitud);
            } else {
                echo "No estas activado";
            }
        }
